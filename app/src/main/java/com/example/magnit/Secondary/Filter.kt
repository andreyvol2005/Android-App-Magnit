...
class Filter : AppCompatActivity() {
    ...
    private fun loadFilters() {
        ...
        chips.values.forEach { findViewById<Chip>(it)?.isChecked = false }
    ...
    private fun save() {
        ...
        ).find { findViewById<Chip>(it.first)?.isChecked == true }?.second ?: "Все"

        prefs.edit().apply {
            putString("selected_category", cat)
            putInt("max_price", b.priceSeekBar.progress)
            putFloat("min_rating", when (b.ratingRadioGroup.checkedRadioButtonId) {
                R.id.rating4Plus -> 4f
            ...
            putInt("min_reviews", when (b.reviewsRadioGroup.checkedRadioButtonId) {
